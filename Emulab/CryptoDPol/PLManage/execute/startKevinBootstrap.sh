...
ssh -i $HOME/.ssh/id_rsa -o ConnectTimeout=$SSH_TIMEOUT -o StrictHostKeyChecking=no ${LOGIN_NAME}@$BOOTSTRAP "/proj/abstracts/jre/bin/java -classpath $BINHOME $BOOTSTRAPCLASSNAME -name $BOOTSTRAP -port $BOOTSTRAP_PORT -nbGroups $NB_GROUPS -nbBallots $NB_BALLOTS"
