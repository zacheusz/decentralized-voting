...
do
      ssh -c arcfour -o Compression=no -x -i $HOME/.ssh/id_rsa -o StrictHostKeyChecking=no ${LOGIN_NAME}@$node "echo -e \"\E[32;33mClearing log files on $node\"; rm -f /tmp/*.out" &
done
...
ssh -c arcfour -o Compression=no -x -i $HOME/.ssh/id_rsa -o StrictHostKeyChecking=no ${LOGIN_NAME}@$BOOTSTRAP "rm -f /tmp/*.out" &
