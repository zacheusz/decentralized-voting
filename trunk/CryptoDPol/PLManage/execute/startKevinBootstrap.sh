...
sshpass -e ssh -o StrictHostKeyChecking=no $LOGIN_NAME@$BOOTSTRAP "cd /home/$LOGIN_NAME/myfiles/tmp/$BOOTSTRAP; java -classpath package/$PROJECT_NAME/bin $BOOTSTRAPCLASSNAME -fileName $BOOTSTRAP-$2.out -name $BOOTSTRAP -port $BOOTSTRAP_PORT -nbGroups $NB_GROUPS"
