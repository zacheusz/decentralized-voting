...
public class Voter {
	...
	int i;
	BigInteger[] a = new BigInteger[votecount];
	byte[] challenge;
	BigInteger response;
	BigInteger[] z = new BigInteger[votecount];
	BigInteger[] e = new BigInteger[votecount];
	...
	r_choice = BigInteger.ONE;
	    ...
	    r = new BigInteger (n[power].bitLength (), rand);
if (DEBUG3) {System.out.println ("choice = " + choice);}
if (DEBUG2 || DEBUG3) {System.out.println ("r = " + r);}
        temp1 = pub.g.modPow (pub.votes[choice], n[power]);
        temp2 = r.modPow (n[power-1], n[power]);
        cip = (temp1.multiply (temp2)).mod (n[power]);
if (DEBUG2) {System.out.println ("encrypted vote(" + choice + ") = " + cip);}
...
if (DEBUG2) {System.out.println ("z'[" + i + "] = " + z[i]);}
		e[i] = new BigInteger (pub.signsize, rand);
if (DEBUG2) {System.out.println ("e'[" + i + "] = " + e[i]);}
                temp2 = pub.g.modPow (pub.votes[i], n[power]);
                temp2 = temp2.modInverse (n[power]);
if (DEBUG2) {System.out.println ("vote(" + i + ")^-1 = " + temp2);}
		temp2 = (cip.multiply (temp2)).mod (n[power]);
if (DEBUG2) {System.out.println ("v(" + i + ") = " + temp2);}
		temp2 = temp2.modPow (e[i], n[power]);
if (DEBUG2) {System.out.println ("v(" + i + ")^e = " + temp2);}
		temp2 = temp2.modInverse (n[power]);
if (DEBUG2) {System.out.println ("v(" + i + ")^-e = " + temp2);}
// Changed from n[1] to n[power-1]                   ==========
		a[i] = (temp2.multiply (z[i].modPow (n[power-1],n[power]))).mod 
		                                                         (n[power]);
if (DEBUG2) {System.out.println ("a'[" + i + "] = " + a[i]);}
	    } else {
		temp2 = new BigInteger (n[power].bitLength (), rand);
		while (temp2.compareTo (n[power]) >= 0)
		    temp2 = new BigInteger (n[power].bitLength (), rand);
if (DEBUG2) {System.out.println ("temp2(" + i + ") = " + temp2);}
// Changed from n[1] to n[power-1]   ==========
...
if (DEBUG2) {System.out.println ("challenge(BI): " + temp1);}
	for (i = 0; i < votecount; i++)
	    if (i != choice)
		temp1 = (temp1.subtract (e[i])).mod (pow2);

	e[choice] = temp1;
if (DEBUG2) {System.out.println ("e[" + choice + "] = " + e[choice]);}



	temp2 = r.modPow (e[choice], n[power]);
if (DEBUG2) {System.out.println ("r = " + r);}
if (DEBUG2) {System.out.println ("temp2 = " + temp2);}
if (DEBUG2) {System.out.println ("r_choice = " + r_choice);}
	z[choice] = (r_choice.multiply (temp2)).mod (n[power]);
if (DEBUG2) {System.out.println ("z[" + choice + "] = " + z[choice]);}
