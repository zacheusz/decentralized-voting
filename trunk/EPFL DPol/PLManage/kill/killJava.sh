...
do
      sshpass -e ssh -c arcfour -o Compression=no -x -i $HOME/.ssh/id_rsa -o StrictHostKeyChecking=no ${LOGIN_NAME}@$node "killall -9 java 2> output; if [[ \`wc -l output | cut -d ' ' -f1\` -gt 0 ]]; then echo -e \"\E[35;32mNothing to kill: $node\"; else echo -e \"\E[35;31mKilled java on $node\"; fi; if [[ -e output ]]; then rm output; fi" &
done
...
sshpass -e ssh -c arcfour -o Compression=no -x -i $HOME/.ssh/id_rsa -o StrictHostKeyChecking=no ${LOGIN_NAME}@$BOOTSTRAP "killall java 2>/dev/null;" &
